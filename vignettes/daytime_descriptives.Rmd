...
important to note that their descriptive value may not correspond to statistical
utility the way similar metrics (e.g., SD) do for non-circular data.
...
## Only run the below code if the PAutilities package is installed
...
circular (i.e., `daytime`) data. Although the descriptive value is not
necessarily coupled with statistical value the way it is for non-circular data,
these features of the `daytime` package are nevertheless valuable for adding
